// componente lista de clientes con shadow DOM
class ClientesList extends HTMLElement {
  constructor() {
    super();
  ...
  render() {
    this.shadowRoot.innerHTML = `
            ...
            <div class="search-container">
                ...
                ${
                  this._filtro
                    ? `<p class="results-count">${this._clientesFiltrados.length} resultado(s) encontrado(s)</p>`
                    : ""
                }
            </div>
            ...
            <div class="table-container">
                ${
                  this._clientesFiltrados.length > 0
                    ? `
                    <table>
                        <thead>
                            <tr>
    ...
    this.shadowRoot.querySelectorAll(".btn-delete").forEach((btn) => {
      btn.addEventListener("click", () => {
        this.dispatchEvent(
          new CustomEvent("delete-cliente", {
            detail: { id: parseInt(btn.dataset.id) },
          })
        );
      });
  ...
  renderTabla() {
    ...
    if (this._filtro) {
      ...
      countEl.textContent = `${this._clientesFiltrados.length} resultado(s) encontrado(s)`;
